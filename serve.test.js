...
test("PWA 1.0.28 发布铭文性能优化与响应式表格资源", async () => {
  ...
  await withServer(async (port) => {
    ...
    assert.match(index.body, /id="pwa-version">1\.0\.28<\/strong>/);
    assert.match(worker.body, /CACHE_NAME\s*=\s*CACHE_PREFIX\s*\+\s*"1\.0\.28"/);
    assert.match(pwa.body, /APP_VERSION\s*=\s*"1\.0\.28"/);
