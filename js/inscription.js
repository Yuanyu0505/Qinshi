(function () {
  ...
  var SUBS = {
    "天盾": [{ n: "3攻", x: true }, { n: "暴击伤害" }, { n: "技能减免" }, { n: "内力" }, { n: "追加伤害" }],
    "地盾": [{ n: "PVP速" }, { n: "暴击" }, { n: "PVP免伤" }, { n: "3技能穿透", x: true }, { n: "防御" }],
    "人盾": [{ n: "3血", x: true }, { n: "抗暴击" }, { n: "PVP伤害" }, { n: "技能穿透" }, { n: "防御" }],
    "神盾": [{ n: "PVP速" }, { n: "3攻", x: true }, { n: "3PVP伤害", x: true }, { n: "技能穿透" }, { n: "内力" }],
    "鬼盾": [{ n: "3速", x: true }, { n: "血" }, { n: "护盾" }, { n: "暴击伤害" }, { n: "防御" }],
    "龙盾": [{ n: "3PVP速", x: true }, { n: "血" }, { n: "3暴击", x: true }, { n: "PVP伤害" }, { n: "技能减免" }],
    "虎盾": [{ n: "3血", x: true }, { n: "暴击伤害" }, { n: "抗暴击" }, { n: "3内力", x: true }, { n: "追加伤害" }],
    "风盾": [{ n: "攻" }, { n: "暴击" }, { n: "护盾" }, { n: "3PVP免伤", x: true }, { n: "追加伤害" }],
    "云盾": [{ n: "3护盾", x: true }, { n: "3抗暴击", x: true }, { n: "PVP免伤" }, { n: "3技能减免", x: true }, { n: "防御" }]
  };
  ...
  function init() {
    ...
    el.progressList.addEventListener("click", editSaved);
  ...
  function renderEditor(item) {
    ...
    el.editor.innerHTML = '<section class="ins-editor"><div class="ins-card-head"><div><span class="ins-quality ' + (item.quality === "红色神将" ? "red" : "orange") + '">' + item.quality + '</span><b>' + escapeHtml(item.name) + '</b></div><button type="button" class="link-btn" data-action="cancel">取消</button></div>' + item.slots.map(function (slot) {
      ...
      var sub = old ? old.sub : SUBS[slot.shield][0].n;
      return '<div class="ins-editor-slot" data-tian="' + slot.tian + '" data-shield="' + slot.shield + '"><div class="ins-slot-title">' + slot.tian + ' · ' + slot.shield + '</div><div class="ins-editor-controls"><label>品质<select class="ins-edit-quality"><option' + (quality === "橙色" ? " selected" : "") + '>橙色</option><option' + (quality === "紫色" ? " selected" : "") + '>紫色</option></select></label><label>星级<select class="ins-edit-star"><option value="2"' + (star === "2" ? " selected" : "") + '>2星</option><option value="1"' + (star === "1" ? " selected" : "") + '>1星</option></select></label><label>副属性<select class="ins-edit-sub">' + SUBS[slot.shield].map(function (attr) { return '<option' + (attr.n === sub ? " selected" : "") + '>' + attr.n + (attr.x ? "（极致）" : "") + '</option>'; }).join("") + '</select></label></div><div class="ins-main-preview"></div></div>';
    }).join("") + '<div class="ins-editor-actions"><button type="button" class="seg active" data-action="save">保存</button></div></section>';
    refreshEditorMain();
  }
  function refreshEditorMain() {
  ...
  function handleEditorClick(event) {
    ...
    progress[editingKey] = { name: item.name, quality: item.quality, slots: Array.prototype.map.call(el.editor.querySelectorAll(".ins-editor-slot"), function (row) {
      return { tian: row.dataset.tian, shield: row.dataset.shield, quality: row.querySelector(".ins-edit-quality").value, star: row.querySelector(".ins-edit-star").value, sub: row.querySelector(".ins-edit-sub").value.replace("（极致）", "") };
    }) };
    saveProgress();
    editingKey = "";
    el.editor.innerHTML = "";
  ...
  function renderProgressList() {
    ...
    el.progressList.innerHTML = entries.length ? entries.map(function (entry) {
      return '<article class="ins-progress-card"><div class="ins-card-head"><div><span class="ins-quality ' + (entry.item.quality === "红色神将" ? "red" : "orange") + '">' + entry.item.quality + '</span><b>' + escapeHtml(entry.item.name) + '</b></div><button type="button" class="seg" data-edit-key="' + encodeURIComponent(entry.key) + '">编辑</button></div><div class="ins-saved-slots">' + entry.saved.slots.map(savedSlotHtml).join("") + '</div></article>';
  ...
  function savedSlotHtml(slot) {
    ...
    return '<div class="ins-saved-slot"><b>' + slot.tian + ' · ' + slot.shield + '</b><span>' + slot.quality + ' ' + slot.star + '星</span><span>' + main + '</span><span>副属性：' + escapeHtml(slot.sub) + '</span></div>';
  }
  function editSaved(event) {
    var button = event.target.closest("button[data-edit-key]");
    if (!button) return;
  ...
  function queryCardHtml(item) {
    ...
    return '<article class="ins-query-card' + (saved ? " saved" : "") + '"><div class="ins-card-head"><div><span class="ins-quality ' + (item.quality === "红色神将" ? "red" : "orange") + '">' + item.quality + '</span><b>' + escapeHtml(item.name) + '</b>' + (saved ? '<span class="ins-saved-mark">个人进度已保存</span>' : "") + '</div></div><div class="ins-query-slots">' + item.slots.map(function (slot) {
      return '<div class="ins-query-slot' + (slotMatchesFilter(slot) ? " matched" : "") + '"><div class="ins-slot-title">' + slot.tian + ' · ' + slot.shield + '</div><div class="ins-main-line"><span>橙色二星主属性</span>' + MAIN[slot.tian]["2"] + '</div><div class="ins-sub-list"><span class="ins-sub-label">可洗练副属性</span>' + SUBS[slot.shield].map(subTagHtml).join("") + '</div></div>';
    }).join("") + '</div></article>';
  }
  function subTagHtml(attr) { return '<span class="ins-sub-tag' + (attr.x ? " extreme" : "") + '">' + attr.n + (attr.x ? '<em>极致</em>' : "") + '</span>'; }

  function renderReference() {
    el.reference.innerHTML = '<section class="ins-reference-block"><h3>天位主属性</h3><div class="table-wrap"><table class="ins-reference-table"><thead><tr><th>天位</th><th>橙色一星</th><th>橙色二星</th><th>紫色</th></tr></thead><tbody>' + TIANS.map(function (tian) { return '<tr><th>' + tian + '</th><td>' + MAIN[tian]["1"] + '</td><td>' + MAIN[tian]["2"] + '</td><td>暂不展示主属性</td></tr>'; }).join("") + '</tbody></table></div></section><section class="ins-reference-block"><h3>盾位副属性</h3><div class="table-wrap"><table class="ins-reference-table"><thead><tr><th>盾位</th><th>可洗练副属性</th></tr></thead><tbody>' + SHIELDS.map(function (shield) { return '<tr><th>' + shield + '</th><td><div class="ins-sub-list">' + SUBS[shield].map(subTagHtml).join("") + '</div></td></tr>'; }).join("") + '</tbody></table></div><div class="muted-tip">红色“极致”标签表示该盾位可洗练出的极致属性。</div></section>';
  }
