...
test("PWA 1.0.27 发布移动端装备属性分级展示与现有离线资源", async () => {
  ...
  await withServer(async (port) => {
    ...
    assert.match(index.body, /id="pwa-version">1\.0\.27<\/strong>/);
    assert.match(worker.body, /CACHE_NAME\s*=\s*CACHE_PREFIX\s*\+\s*"1\.0\.27"/);
    assert.match(pwa.body, /APP_VERSION\s*=\s*"1\.0\.27"/);
